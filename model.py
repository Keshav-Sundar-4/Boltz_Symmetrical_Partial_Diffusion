...
class Boltz1(LightningModule):
    def __init__(  # noqa: PLR0915, C901, PLR0912
        ...
        chain_symmetry_groups: dict = {},
        radius: Optional[float] = None,  # Add radius here
        no_msa: bool = False,
        ...
        self.structure_module = AtomDiffusion(
            ...
            radius=radius,  # Pass radius to AtomDiffusion
    ...
    def forward(
        ...
        input_coords: Optional[torch.Tensor] = None,  # ADDED input_coords ARGUMENT
    ) -> dict[str, torch.Tensor]:
        # -- ADDED PRINT STATEMENT --
        print("input_coords:", input_coords)

        # --- TEMPORARILY OVERRIDE COORDINATES FOR EMBEDDING COMPUTATION ---
        original_coords = None
        if (not self.training) and input_coords is not None:
            # Instead of simply centering the coordinates,
            # we now assume that 'input_coords' are already mapped from the PDB.
            # We override the features' coordinate field with these mapped coordinates.
            original_coords = feats["coords"].clone()
            # Optionally, if any centering or adjustment is needed, perform it here.
            feats["coords"] = input_coords  # use the mapped coordinates directly
        ...
        # --- INFERENCE (or confidence prediction) BRANCH: REVERT COORDINATES ---
        if (not self.training) or self.confidence_prediction:
            if input_coords is not None:
                input_coords = input_coords.squeeze(1)
            if (not self.training) and (original_coords is not None):
                feats["coords"] = original_coords
            dict_out.update(
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> Any:
        try:
            # Use the model's input_coords (PDB-derived) if available.
            pdb_coords = getattr(self, "input_coords", None)
            if pdb_coords is not None:
                input_coords = pdb_coords  # shape: [B, A, 3]
                # Determine expected number of atoms from the YAML-derived coordinates.
                if batch["coords"].ndim == 4:
                    # Assume shape is [B, C, A, 3], use dimension 2.
                    expected_atoms = batch["coords"].shape[2]
                elif batch["coords"].ndim == 3:
                    # Assume shape is [B, A, 3]
                    expected_atoms = batch["coords"].shape[1]
                elif batch["coords"].ndim == 2:
                    expected_atoms = batch["coords"].shape[0]
                else:
                    expected_atoms = None

                if expected_atoms is not None:
                    current_atoms = input_coords.shape[1]
                    if current_atoms > expected_atoms:
                        # Crop the extra atoms.
                        input_coords = input_coords[:, :expected_atoms, :]
                    elif current_atoms < expected_atoms:
                        # Pad by repeating the last coordinate.
                        pad_length = expected_atoms - current_atoms
                        pad_tensor = input_coords[:, -1:, :].repeat(1, pad_length, 1)
                        input_coords = torch.cat([input_coords, pad_tensor], dim=1)
            else:
                input_coords = batch.get('coords', None)
                if input_coords is not None and input_coords.ndim == 3:
                    input_coords = input_coords.squeeze(1)

            # Print sizes for debugging.
            print("YAML coords size:", batch["coords"].shape, "PDB-derived input_coords size:", input_coords.shape)
            ...
            out = self(
                ...
                input_coords=input_coords,  # Use the adjusted PDB-derived coordinates.
            )
            ...
            if "out of memory" in str(e):
                ...
                raise Exception("exception: True")
